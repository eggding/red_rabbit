...
def OnPlayerEnterScene(nPlayerGID, szSrcScene, dictSerialData):
    ...
    if szSrcScene == scene_def.LOGIN_SCENE:
        ...
        dbs_client.DoAsynCall(rpc_def.DbsLoadPlayerData, nPlayerGID, funCb=OnLoadPlayerDataDone, nChannel=nPlayerGID)
    else:
...
def Gac2RoomServiceQueryAll(session, dictData):
    ...
    dbs_client.DoAsynCall(rpc_def.DbsTest, session, OnDbsTestCb, nChannel=session)
