...
def _update_function(oldobj, newobj, depth):
    ...
    if hasattr(newobj, "func_closure_for_reload") and hasattr(oldobj, "func_closure_for_reload"):
        _update_function(oldobj.func_closure_for_reload, newobj.func_closure_for_reload, depth + 1)

    if hasattr(newobj, "cls_closure_for_reload") and hasattr(oldobj, "cls_closure_for_reload"):
        _update_new_style_class(oldobj.cls_closure_for_reload, newobj.cls_closure_for_reload, depth + 1)
...
def get_valid_handlers(is_update_data=False):
    ...
    adict = {
            ...
            types.ClassType: _update_old_style_class,  #    types.DictType : _update_dict,
...
def update(module_name, is_update_data=False):
    ...
    for key, newobj in new_module.__dict__.iteritems():
        ...
        if id(oldobj) == id(newobj):  #同一个对象就不用执行update了. by jmhuo, 比如import进来的东东
            continue
        ...
        if type(newobj) != type(oldobj):
            if isinstance(newobj, types.TypeType) and isinstance(oldobj, types.TypeType):
                #虽然类型不同，但是都是类定义，很有可能是类和Metaclass在同一个文件定义。还是给予handler
                pass
            else:
                new_module.__dict__[key] = oldobj  # 特殊地，这里需要替换，因为reload后的module对象和reload前是同一个地址
                _log("[RD] %s : %s" % (key, _S(oldobj)), depth)
                print "[reload-warning]type(newobj) != type(oldobj), so to keep use oldobj. newobj:[%s]%s " % (key, newobj)
...
def updateObj(oldobj,newobj,is_update_data=False):
    depth = 1
    handlers = get_valid_handlers(is_update_data)

    if id(oldobj) == id(newobj):
        print "updateObj error id(oldobj) == id(newobj)"
        return
    ...
    if type(newobj) != type(oldobj):
        if isinstance(newobj, types.TypeType) and isinstance(oldobj, types.TypeType):
            ...
            print "updateObj error type(newobj) != type(oldobj),new(%s),old(%s)" % (type(newobj),type(oldobj))
            return
    ...
    if handler:
        ...
        print "updateObj error  no handler for newobj:%s, to use oldobj. " %  newobj

def updateFuncCode(oldobj,newobj):
    oldT=type(oldobj)
    newT=type(newobj)
    print "updateFuncCode oldobj(%s),newobj(%s) " % (oldT,newT)
    ...
    if oldT == types.FunctionType:
        oldFC=oldobj
    elif oldT == types.MethodType:
        oldFC=oldobj.im_func
    elif oldT == _StaticMethodType:
        oldFC=oldobj.im_func
    elif oldT == _ClassMethodType:
        oldFC = oldobj.__get__(object).im_func

    if newT == types.FunctionType:
        newFC=newobj
    elif newT == types.MethodType:
        newFC=newobj.im_func
    elif newT == _StaticMethodType:
        newFC=newobj.im_func
    elif newT == _ClassMethodType:
        newFC = newobj.__get__(object).im_func

    if oldFC and newFC:
        _update_function(oldFC,newFC,1)
    else:
        print "updateFuncCode error oldobj(%s),newobj(%s) " % (oldT,newT)
